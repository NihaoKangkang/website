...
function init() {  //初始化
  ...
  time = diff.children[1].value - 2    //游戏难度(时间间隔)
  direction = "before"                //默认前进方向
  coordinate_X = [0, 20]                  //蛇的X坐标数组[0, 20] 初始长度为2
  coordinate_Y = [320, 320]                //蛇的Y坐标数组[320, 320]
  ...
  // 添加初始的蛇身部件
  for (let i = 1; i < coordinate_X.length; i++) {
    let Snak_body = document.createElement("div")
    Snak_body.className = "Snak_body"
    Snak_body.style.left = coordinate_X[i] + "px"
    Snak_body.style.top = coordinate_Y[i] + "px"
...
function move_direction() {  //移动以及移动过程中的事件判断
  ...
  if (direction === "up") {
    Snak_head.style.top = (coordinate_Y[0] - 20 + 640) % 640 + "px"
    coordinate_Y[0] = (coordinate_Y[0] - 20 + 640) % 640
  } else if (direction === "down") {
    Snak_head.style.top = (coordinate_Y[0] + 20) % 640 + "px"
    coordinate_Y[0] = (coordinate_Y[0] + 20) % 640
  } else if (direction === "before") {
    Snak_head.style.left = (coordinate_X[0] + 20) % 640 + "px"
    coordinate_X[0] = (coordinate_X[0] + 20) % 640
  } else if (direction === "after") {
    Snak_head.style.left = (coordinate_X[0] - 20 + 640) % 640 + "px"
    coordinate_X[0] = (coordinate_X[0] - 20 + 640) % 640
  } else {
    game_over()
    return
  }
...
function game_over_judge() {    //游戏结束判断
  ...
  for (let i = 0; i < judge_X.length; i++) {
    if (judge_X[i] === coordinate_X[0] && judge_Y[i] === coordinate_Y[0]) return false
  }
  return true
}
